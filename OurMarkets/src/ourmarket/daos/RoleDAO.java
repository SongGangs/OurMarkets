...
public class RoleDAO extends BaseHibernateDAO {
	...
	public Role findById(java.lang.Short id) {
		...
		try {
			Role instance = (Role) getSession().get("ourmarket.models.Role", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Role instance) {
		...
		try {
			List results = getSession().createCriteria("ourmarket.models.Role").add(Example.create(instance)).list();
	...
	public List findByRinfo(Object rinfo) {
		return findByProperty(RINFO, rinfo);
	}
