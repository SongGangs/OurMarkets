...
public class MessageDAO extends BaseHibernateDAO {
	...
	public Message findById(java.lang.Integer id) {
		...
		try {
			Message instance = (Message) getSession().get("ourmarket.models.Message", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Message instance) {
		...
		try {
			List results = getSession().createCriteria("ourmarket.models.Message").add(Example.create(instance)).list();
