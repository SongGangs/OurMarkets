 ...
 */
@Repository
public class DetailMessageDAO extends BaseHibernateDAO {
	...
	public DetailMessage findById(java.lang.Long id) {
		...
		try {
			DetailMessage instance = (DetailMessage) getSession().get("ourmarket.models.DetailMessage", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(DetailMessage instance) {
		...
		try {
			List results = getSession().createCriteria("ourmarket.models.DetailMessage").add(Example.create(instance))
					.list();
	...
	public DetailMessage merge(DetailMessage detachedInstance) {
		...
		try {
			DetailMessage result = (DetailMessage) getSession().merge(detachedInstance);
